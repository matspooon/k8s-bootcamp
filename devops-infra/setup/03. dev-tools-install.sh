...
helm upgrade --install gitea gitea-charts/gitea \
  --namespace dev-tools \
  --create-namespace \
  -f $PROFILE/gitea-values.yaml
# helm chart upgrade
#helm upgrade gitea gitea-charts/gitea -n dev-tools -f $PROFILE/gitea-values.yaml

# jenkins 설치
helm repo add jenkins https://charts.jenkins.io
helm repo update
helm upgrade --install jenkins jenkins/jenkins -n dev-tools -f $PROFILE/jenkins-values.yaml
#helm upgrade jenkins jenkins/jenkins -n dev-tools -f $PROFILE/jenkins-values.yaml
kubectl apply -f jenkins-pvc.yaml


# jenkins에서 pull하는 docker image가 docker desktop의 images에서 pull할 것이므로 skip
if false; then
...
docker push gitea.k8s.dev/admin/gcr.io/kaniko-project/executor:latest
  ...
  echo ">>> gitea docker registry에 image 설치 스킵..."
fi
...
helm repo update
helm upgrade --install argocd argo/argo-cd -n dev-tools -f $PROFILE/argocd-values.yaml
# admin 비번확인
kubectl -n dev-tools get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d
